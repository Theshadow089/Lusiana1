...
function CreateWait(source, name, reason, coords)
    Draw3DTextTimeout({
        ...
        timeout=60000,
        text={
            content='SPIELER ~b~[~y~'..source..'~b~] ~b~'..name..'~s~ HAT DEN SERVER VERLASSEN\n~o~GRUND~s~: ~r~'..reason,
            rgb = {255 , 255, 255},
            textOutline=true,
            scaleMultiplier=1,
            font=0
...
RegisterNetEvent('beans_Disconnect:sendPlayerDropped')
AddEventHandler('beans_Disconnect:sendPlayerDropped', function(data)
